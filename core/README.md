...
V.I.S.I.O.N. is a simple, open-source API for head tracking and dwell clicking that you can add to any web application.
...
The dwell clicker can also work independently of the head tracking, for use with external pointing devices, including eye trackers (which require similar UI concerns), and the V.I.S.I.O.N. desktop app, which can control your computer's mouse.
...
[✨👉 **Demo and more information on the V.I.S.I.O.N. website** 👈✨](https://Vision.js.org/)

## Installation

```bash
npm install vision-hft
```
...
```html
<script src="path/to/vision-hft/vision.js"></script>
```
...
```javascript
Vision.dependenciesRoot = "path/to/vision-hft";
```

You also need to include the stylesheet, which is in the same directory as the script.

```html
<link rel="stylesheet" href="path/to/vision-hft/vision.css">
```

## Head Tracking

V.I.S.I.O.N. makes it easy to set up head tracking,
...
and in the future the library should help you with this, but for now you'll have to do it yourself, by defining a callback `Vision.onPointerMove(x, y)`.

You can copy this code to get started:

```javascript
Vision.loadDependencies().then(function() {
	Vision.init();

	// Pointer event simulation logic should be built into V.I.S.I.O.N. in the future.
	// These simulated events connect the V.I.S.I.O.N. head tracker to the V.I.S.I.O.N. dwell clicker,
...
### `Vision.dependenciesRoot`

Set this to the path to the folder where you installed vision-hft, without a trailing slash.

### `Vision.loadDependencies([options])`
...
### `Vision.init([element, options])`

`Vision.init` initializes the library *for head tracking*. (It is not needed for dwell clicking.)

It creates the UI, either creating a new `<div class="vision-ui">` element and appending it to the `<body>`,
or using, and modifying, and existing element.

If you pass an element, it should be an empty `<div>` element.
It will add `class="vision-ui"` directly to the element if it doesn't already have it.
...
### `Vision.initDwellClicking(config)`
...
- `config.shouldClickThrough(el)` (optional): a function that returns true if the element should be totally ignored, allowing clicking on content behind it. Prefer `pointer-events: none` when possible, which will work for all input methods. Use this only if you need to differentiate input methods. Default: `(el) => el.matches(".vision-click-through, .vision-click-through *")`
...
Example:
```javascript
// This example is based off of how JS Paint uses the V.I.S.I.O.N. API.
...
const dwellClicker = Vision.initDwellClicking(config);
...
### `Vision.initScreenOverlay()` [UNSTABLE]
...
For release notes, see [CHANGELOG.md](https://github.com/1j01/vision/blob/main/CHANGELOG.md)

## License

[MIT License](https://github.com/1j01/vision/blob/main/LICENSE.txt)

## Development

See [Development Setup](https://github.com/1j01/vision#development-setup) in the main README.
