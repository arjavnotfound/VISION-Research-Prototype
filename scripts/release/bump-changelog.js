...
const oldVersionMatch = changelog.match(/\[Unreleased\]: https:\/\/github\.com\/1j01\/vision\/compare\/v([^.]+\.[^.]+\.[^.]+)\.\.\.HEAD/);
if (oldVersionMatch) {
	const previousVersion = oldVersionMatch[1];
	updatedChangelog = updatedChangelog.replace(
		/\[Unreleased\]: https:\/\/github\.com\/1j01\/vision\/compare\/v[^.]+\.[^.]+\.[^.]+\.\.\.HEAD/,
		`[Unreleased]: https://github.com/1j01/vision/compare/v${version}...HEAD\n[${version}]: https://github.com/1j01/vision/compare/v${previousVersion}...v${version}`
	);
}
