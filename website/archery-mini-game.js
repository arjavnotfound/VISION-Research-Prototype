...
cursor_guard.classList.add("vision-click-through"); // only block system cursor, not TM
...
try {
	const json = localStorage.getItem("vision-archery-best-times");
	if (json) {
...
function initRound() {
	...
	try {
		if (localStorage.getItem("vision-archery-debug") === "true") {
			enableDebug = true;
		}
	} catch (_error) {
...
function handleTargetHit(event) {
	...
	if (document.querySelectorAll(".archery-target:not(.hit)").length === 0) {
		...
		try {
			localStorage.setItem("vision-archery-best-times", JSON.stringify(best_times));
		} catch (_error) {
			// ignore localStorage access errors
		}
