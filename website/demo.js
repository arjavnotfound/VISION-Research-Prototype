...
Vision.dependenciesRoot = "./core";

await Vision.loadDependencies();
...
const initOptions = {
	...
	// If you do decide to copy from this demo, make sure to pin vision-hft to
...
Vision.init(document.getElementById("vision-demo"), initOptions);

// UNSTABLE API
const screenOverlay = Vision.initScreenOverlay();
...
dwellClicker = Vision.initDwellClicking(config);

function isEnabled() {
	// HACK, TODO: get state from proper channel
	const toggleButton = document.querySelector(".vision-start-stop-button");
...
function updateDwellClickingEnabled() {
	// This function can be called during the call to Vision.init
	...
	// to support the basic features of V.I.S.I.O.N..
	if (!dwellClicker) return;
	const enabled = isEnabled();
	dwellClicker.paused = activeSettings.clickingMode !== "dwell" || !isClickingAllowed();
	const virtualCursor = document.querySelector(".vision-pointer");
...
function getScreenOverlayMessageText({ isManualTakeback, enabled }) {
	...
	return isManualTakeback ?
		...
		typeof enabled !== "boolean" ? t("hud.pressToToggle", { defaultValue: "Press %0 to toggle V.I.S.I.O.N.." }).replace("%0", "F9") :
			enabled ?
				t("hud.pressToDisable", { defaultValue: "Press %0 to disable V.I.S.I.O.N.." }).replace("%0", "F9") :
				t("hud.pressToEnable", { defaultValue: "Press %0 to enable V.I.S.I.O.N.." }).replace("%0", "F9");
}
