...
function updateSelectedFolderStatus() {
	...
	if (folderName === "poses") {
		selectedFolderStatus.textContent += "\n⚠️ï¸ Please select the parent folder of 'poses'";
	}
...
function init() {
	const Vision = Vision.init(document.getElementById("vision-hft"));
	...
	setInterval(() => {
		if (Vision._facemeshPrediction) {
			recordSnapshot(Vision._facemeshPrediction, Vision._headTilt, Vision._video);
		}
		Vision._setPaused(true);
	}, 100);
}
