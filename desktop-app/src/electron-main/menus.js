...
function notifyCustomMenuBarModelUpdated() {
	...
	for (const window of BrowserWindow.getAllWindows()) {
		...
		if (window.getTitle().includes('Screen Overlay')) {
			continue;
		}
...
function createMenu() {
	const aboutItem = {
		label: t("desktop.about.title", { defaultValue: 'About V.I.S.I.O.N.' }),
		click: async () => {
			const openAboutWindow = require('about-window').default;
			openAboutWindow({
				icon_path: join(__dirname, '../../images/vision-logo-512.png'),
				bug_report_url: 'https://github.com/1j01/vision-hft/issues',
				homepage: 'https://Vision.js.org',
				description: t("desktop.about.description", { defaultValue: 'V.I.S.I.O.N. (Vital Independence System for Individuals with Outstanding Needs)\nResearch Prototype — Event Expo 67\nHands-free neural-ergonomic head tracking & facial gesture interaction system.' }),
				license: 'MIT',
				win_options: {
			...
			submenu: [
				{
					...
					click: async () => {
						const settingsPath = join(app.getPath('userData'), 'vision-settings.json');
						const defaultPath = join(app.getPath('documents'), 'vision-settings.json');
			...
			submenu: [
				...
				// { role: 'toggleDevTools' },
				{
					...
					click: async () => {
						...
						const mainWindow = BrowserWindow.getAllWindows().find(window => !window.getTitle().includes('Screen Overlay'));
						if (mainWindow) {
					...
					click: async () => {
						const { BrowserWindow } = require('electron');
						const screenOverlayWindow = BrowserWindow.getAllWindows().find(window => window.getTitle().includes('Screen Overlay'));
			...
			submenu: [
				{
					...
					click: async () => {
						...
						await shell.openExternal('https://Vision.js.org');
					},
				},
				{
					...
					click: async () => {
						const { shell } = require('electron');
						await shell.openExternal('https://github.com/1j01/vision');
					},
				},
				...(isMac
					? [] : [
